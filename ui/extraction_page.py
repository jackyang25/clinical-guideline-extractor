"""Extraction pipeline UI for processing PDFs."""
...
def render_extraction_page(base_output_dir: Path, prompt_path: Path) -> None:
    """Render the extraction pipeline page."""
    ...
    # output settings (session-specific paths)
    ...
    if "extraction_results" in st.session_state and st.session_state.extraction_results:
        _display_results(st.session_state.extraction_results)
...
def _display_results(results: dict) -> None:
    ...
    st.success(f"Files saved to: ./output/{results['session_id']}/")
    
    st.subheader("Guideline-level metadata")
    
    st.caption("Extracted Metadata (AI-extracted from cover page)")
    st.json({
    ...
    with col_dl1:
        st.download_button(
            ...
            help="Flat array of self-contained chunks. Use for RAG systems or database import.",
        )
    with col_dl2:
        st.download_button(
            ...
            mime="application/json",
            use_container_width=True,
            help="Hierarchical structure: guideline → pages → chunks. Use for rendering complete guidelines.",
        )
